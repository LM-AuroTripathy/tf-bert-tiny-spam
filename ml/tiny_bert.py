...
def create_model(max_len, classifier_layer=True):
    ...
    # Setup input layer
    input_ids = layers.Input(batch_size=eval_batch_size,
                             shape=(max_len,),
                             dtype=tf.int32, name="input_ids")
    token_type_ids = layers.Input(batch_size=eval_batch_size,
                                  shape=(max_len,),
                                  dtype=tf.int32, name="token_type_ids")
    attention_mask = layers.Input(batch_size=eval_batch_size,
                                  shape=(max_len,),
...
def run():
    ...
    spam_file = "./examples/browser-bert/ml/spam.csv"
